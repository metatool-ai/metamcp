...
export class Logger {
    ...
    private customLog(outputStream: WriteStream, level: 'DEBUG' | 'INFO' | 'WARN' | 'ERROR', ...args: unknown[]) {
        ...
        if (this.consoleMode !== 'none') {
            const shouldMirror =
                ...
                (this.consoleMode === 'errors-only' && (level === 'WARN' || level === 'ERROR'));
    ...
    public debug = (...args: unknown[]) => this.customLog(this.logFile, 'DEBUG', ...args);
    public info = (...args: unknown[]) => this.customLog(this.logFile, 'INFO', ...args);
    public warn = (...args: unknown[]) => this.customLog(this.logFile, 'WARN', ...args);
    public error = (...args: unknown[]) => this.customLog(this.errorFile, 'ERROR', ...args);
